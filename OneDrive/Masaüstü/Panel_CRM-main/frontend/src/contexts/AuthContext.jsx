...
export const AuthProvider = ({ children }) => {
    ...
    const loginAction = async (email, password) => {
        ...
        try {
            ...
            localStorage.setItem('kolayPanelToken', data.token);
            localStorage.setItem('kolayPanelUser', JSON.stringify({
    ...
    const signupAction = async (username, email, password, role = 'Sales') => {
        setAuthState(prev => ({ ...prev, isLoading: true, error: null }));
        try {
            ...
            setAuthState({
                token: data.token,
                user: { username: data.username, email: data.email, role: data.role, id: data._id },
                isAuthenticated: true,
                isLoading: false,
                error: null
            });
